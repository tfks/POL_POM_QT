#ifndef BOUQUETMANAGERPLUGIN_H
#define BOUQUETMANAGERPLUGIN_H

#include <QObject>
#include <QtPlugin>
#include <QObjectList>

#include "../../POL_POM_QT/src/headers/plugininterface.h"
#include "../../POL_POM_QT/src/headers/mainwindow.h"

#include "../../POL_POM_QT_LIB_VIRTUALDRIVE/virtualdriveitem.h"

class BouquetManagerPlugin : public QObject, public IPluginInterface
{
    Q_OBJECT
    Q_PLUGIN_METADATA(IID "playonlinux.IPluginInterface" FILE "POL_POM_QT_PLUGINS_BOUQUETMANAGER.json")
    Q_INTERFACES(IPluginInterface)
public:
    BouquetManagerPlugin();
    ~BouquetManagerPlugin();

    bool connectPlugin(MainWindow *mainWindow) Q_DECL_OVERRIDE;

private:
    MainWindow *mainWindow;
    QList<VirtualDriveItem*> listOfVirtualDrives;

    QToolButton *addVirtualDriveButton;

    void getListOfVirtualDrives();
    void createAddVirtualDriveButton();

signals:
    void signal_VirtualDriveListChange(QList<VirtualDriveItem*> virtualDriveItems);
    void signal_addNewVirtualDriveButton(QToolButton *addVirtualDriveButton);

private slots:
    void slot_mainWindowIsShown();

    void slot_actionAddNewVirtualDrive_triggered();
};

#endif // BOUQUETMANAGERPLUGIN_H
